...
puts get_shakey.keys
puts get_shakey["William Shakespeare"]
puts("-----------------------")
puts get_shakey.values #gives the same result as the puts statement below
=begin
get_shakey.values.each {|x,y| #Why is it that the keys and values are not beinbg assigned properly ?
puts x
}
=end
get_shakey["William Shakespeare"].each {|key,val|
#puts val
puts("#{val["title"]}" + "," + "#{val["finished"]}")
}
