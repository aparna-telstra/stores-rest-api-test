=begin
puts s.keys
puts s["William Shakespeare"]
puts("-----------------------")
puts s.values #gives the same result as the puts statement below

#s.values.each {|x,y| #Why is it that the keys and values are not beinbg assigned properly ?
#puts x
#}
...
def print_plays(from_yr,to_yr)
  ...
  puts v
  from_yr <= v["finished"] && to_yr >= v["finished"] }.each {|key,val| #I dont understand from_yr and to_yr here
    puts "#{val["title"].ljust(30)} #{val["finished"]}"
  }

end

print_plays(1599,1605)
